...
import type { ResponderEvent, ResponderProps } from '#exports/useResponder';

import React, { useDebugValue } from 'react';
import TouchHistoryMath from '#internals/TouchHistoryMath';
...
type ActiveCallback = (
  ...
  gestureState: PanState
) => boolean;

type PassiveCallback = (event: ResponderEvent, gestureState: PanState) => void;
...
export type PanProps = {|
  ...
  touchAction?: $PropertyType<ResponderProps, 'touchAction'>
|};
 ...
 */
function initializeGestureState(gestureState: PanState) {
  gestureState.x = 0;
  gestureState.y = 0;
...
export function usePan(config: PanProps): CallbackRef {
  const {
    onStartShouldSetPan,
  ...
  const gestureStateRef = React.useRef<PanState>({
