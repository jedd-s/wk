...
const modality = (insertRule) => {
  ...
  function focusTriggersKeyboardModality(el) {
    ...
    if (tagName === 'INPUT' && inputTypesAllowlist.has(type) && !isReadOnly) {
      return true;
    }
  ...
  function onFocus(e) {
    ...
    if (
      getInputModality() === 'keyboard' ||
      focusTriggersKeyboardModality(target)
    ) {
  ...
  document.addEventListener('react:modalitychange', () => {
    const inputModality = getInputModality();
