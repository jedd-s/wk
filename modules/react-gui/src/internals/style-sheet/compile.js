...
import { prefixStyles, prefixInlineStyles } from './prefixStyles';
...
export function classic(style: Style, name: string): CompilerOutput {
  ...
  if (animationKeyframes != null) {
    const { animationNames, rules: keyframesRules } = processKeyframesValue(
      animationKeyframes
    );
...
function createAtomicRules(identifier: string, property: string, value): Rules {
  ...
  switch (property) {
    ...
    // See #513
    case 'pointerEvents': {
      const pointerEvents: string = property; // for flow
